...
# ─────────────────────────────────────────────
# Stats Modal (st.dialog)
# ─────────────────────────────────────────────
@st.dialog("📊 סטטיסטיקת בחירות כלל המשתמשים", width="large")
def _stats_modal():
...
if active_tab == "⚙️ הגדרות יעד":
    ...
    # ── Inline Results (shown in same tab after calculation) ──
    ...
    if res is not None and active_tab == "⚙️ הגדרות יעד":
        ...
        if recs:
            ...
            header_html = (
                f"<tr><th style='min-width:110px'>חלופה</th>"
                f"<th style='min-width:180px'>מסלולים ומשקלים</th>"
                f"{''.join(f'<th>{c}</th>' for c in num_cols)}"
                f"{change_th}</tr>"
            )

            def _make_row(label, d, row_style="", label_style="", is_alt=False):
                ...
                for col_lbl, col_key in zip(num_cols, row_keys):
                    if d is None:
                        ...
                        # ── Sharpe incomplete: show warning instead of value ──
                        if col_key == "שארפ משוקלל" and is_alt:
                            if d.get("sharpe_incomplete") or (isinstance(val, float) and math.isnan(val)):
                                cells += "<td style='font-size:10px;color:#92400e'>לא כל המסלולים פרסמו שארפ</td>"
                                continue
                        dstyle = ""
                ...
                if show_changes:
                    if is_alt:
                        cells += f"<td style='font-size:11px;line-height:1.8'>{_count_changes(d)}</td>"
                    else:
                        cells += "<td>—</td>"
            ...
            tgt_data = {
                ...
                'מט"ח (%)': targets_used.get("fx","—"), "לא־סחיר (%)": targets_used.get("illiquid","—"),
            ...
            if has_baseline:
                bl_data = {
                    ...
                    'מט"ח (%)': baseline.get("fx","—"), "לא־סחיר (%)": baseline.get("illiquid","—"),
            ...
            for i, rrow in enumerate(rows):
                body_html += _make_row(
                    ...
                    label_style="font-weight:900;color:#0f1e3d", is_alt=True,
                )

            st.markdown(f"""
<div style='overflow-x:auto;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;margin-bottom:14px'>
  <table class='res-tbl'>
    ...
    <tbody>{body_html}</tbody>
            ...
            if not show_changes and not has_baseline:
                st.caption("💡 העלה דו\"ח מסלקה בטאב 'פורטפוליו נוכחי' כדי לראות השוואה ועמודת שינויים נדרשים")
            ...
            # ── Alternative selection → opens stats modal ──
            st.markdown("---")
            voting_configured = hasattr(st, "secrets") and "gcp_service_account" in st.secrets

            st.markdown("""
<div class='vote-section'>
  <div class='vote-title'>🗳️ בחר חלופה מועדפת</div>
  <div class='vote-sub'>לחץ על חלופה כדי לבחור אותה ולראות סטטיסטיקות כלל המשתמשים (אנונימי)</div>
</div>""", unsafe_allow_html=True)

            voted_key = "voted_this_session"
            current_vote = st.session_state.get(voted_key)

            if current_vote:
                vc1, vc2, vc3 = st.columns([2, 1, 1])
                with vc1:
                    st.markdown(f"<span class='voted-badge'>✅ בחרת: {current_vote}</span>", unsafe_allow_html=True)
                with vc2:
                    if st.button("📊 פתח סטטיסטיקות", use_container_width=True, key="open_stats_btn"):
                        _stats_modal()
                with vc3:
                    if st.button("🔄 שנה בחירה", use_container_width=True, key="change_vote_btn"):
                        st.session_state[voted_key] = None
                        st.rerun()
            else:
                vote_cols = st.columns(4)
                vote_icons = ["⭐", "🎯", "📈", "🤝"]
                for col, rrow, icon in zip(vote_cols, rows, vote_icons):
                    with col:
                        label = rrow["חלופה"]
                        ...
                        if st.button(
                            ...
                            key=f"vote_{label}",
                            help="לחץ לבחור חלופה זו ולראות סטטיסטיקות"
                        ):
                            if voting_configured:
